...
def Oblivious(k, model) :
    ...
    while len(S) < k :
        ...
        # find maximum element that was not already added
        max = argmax_with_condition(grad, T)
        if models.constraint(S, int(max), model, 'Top_k', k) :
            S = np.append(S, int(max))
        T = np.append(T, int(max))
        rounds_ind += 1
        
        #print(T.size)
        
        if grad.size == len(T) : break
